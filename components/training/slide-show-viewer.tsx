...
interface SlideShowViewerProps {
  ...
  slideCount: number
  title: string
  /** When provided, show Download in chrome and in error state. Omit for company-private (view only). */
  downloadUrl?: string
...
export function SlideShowViewer({
  ...
  const hasDownload = Boolean(downloadUrlProp)
      ...
      >
        ...
        {imageError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-slate-900/95 p-6 text-center text-slate-300">
            ...
            <p className="text-slate-400 text-sm">Try again later.</p>
            {hasDownload && (
              <a
                href={downloadUrlProp}
                download
                target="_blank"
